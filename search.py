...
def graph_search(problem, fringe, max_expanded=10000):
    """Search through the successors of a problem to find a goal.
    The argument fringe should be an empty queue.
    ...
    expanded = 0
    fringe.append(Node(problem.initial))
    while fringe and expanded < max_expanded:
        node = fringe.pop()
        if problem.goal_test(node.state):
...
def best_first_graph_search(problem, f, max_expanded=10000):
    ...
    f = memoize(f, 'f')
    return graph_search(problem, PriorityQueue(min, f), max_expanded)
...
def astar_search(problem, h=None, max_expanded=10000):
    ...
    return best_first_graph_search(problem, f, max_expanded)
